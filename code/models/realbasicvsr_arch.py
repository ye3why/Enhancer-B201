...
class ConvAct(nn.Module):
    def __init__(self,in_channels=8, out_channels=32, kernel_size=7, stride=1, padding=3, useact=True):
    ...
    def forward(self, tensor_input):
        if self.useact:
            return self.act(self.conv(tensor_input))
        else:
            return self.conv(tensor_input)
...
class BasicModule(nn.Module):
    ...
    def __init__(self):
        ...
        self.basic_module = nn.Sequential(
            ConvAct(in_channels=8, out_channels=32, kernel_size=7, stride=1, padding=3),
            ConvAct(in_channels=32, out_channels=64, kernel_size=7, stride=1, padding=3),
            ConvAct(in_channels=64, out_channels=32, kernel_size=7, stride=1, padding=3),
            ConvAct(in_channels=32, out_channels=16, kernel_size=7, stride=1, padding=3),
            ConvAct(in_channels=16, out_channels=2, kernel_size=7, stride=1, padding=3, useact=False))
