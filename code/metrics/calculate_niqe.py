...
class NIQEWorker(EvalWorker):
    def __init__(self, que, qid, **kwargs):
        super(NIQEWorker, self).__init__(que, qid)
    ...
    def eval_func(self, res, name, restored, gt=None):
        img = restored.astype(np.float32)
        ...
        mu_pris_param = self.niqe_pris_params['mu_pris_param']
        cov_pris_param = self.niqe_pris_params['cov_pris_param']
        gaussian_window = self.niqe_pris_params['gaussian_window']
        res[name] = niqe(img, mu_pris_param, cov_pris_param, gaussian_window)
...
def parse_args():
    ...
    parser.add_argument('--restored', type=str, default='datasets/val_set14/Set14', help='Input path')
...
def main(args):
    ...
    niqe_all = []
    img_list = sorted(scandir(args.restored, recursive=True, full_path=True))
