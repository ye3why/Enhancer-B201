...
## 介绍
易用、高效、可扩展的视频增强框架, 支持多种增强功能，如超分辨率、去噪、去压缩失真、SDR转HDR等

### 特性

- 易用，仅需选择模型并设置少量参数，运行即可直接输出增强后的视频

- 高效，多线程、gpu增强和视频压缩同步进行、利用内存暂存结果等方式有效缓解了IO瓶颈

- 支持断点，自动保存分段结果

- 可扩展，模型、数据读写和视频增强流程解耦，新模型仅需在models.yml中添加即可使用

- 其他功能，如添加胶片噪声，对图片数据集测试等
...
若使用视频增强模型，图片文件名需要是数字，并且从1开始递增, 1, 2, 3, 4, 5 ,6...

单图输入的模型，不要求图片文件名按照以上规则
```bash
